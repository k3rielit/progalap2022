...
float t(trapez tr) {
  return ((tr.a+tr.c)*tr.m)/2;
}

trapez compare(trapez tr1, trapez tr2) {
  return t(tr1) == t(tr2) ? tr1 : (t(tr1)>t(tr2) ? tr1 : tr2);
}
...
void capitalize(char bemenet[], char eredmeny[]) {
  int i=0, isFirst=1;
  for(; bemenet[i]; i++) {
    if(bemenet[i]>='a' && bemenet[i]<='z' && isFirst) eredmeny[i]=bemenet[i]-32;
    else if(bemenet[i]>='A' && bemenet[i]<='Z' && !isFirst) eredmeny[i]=bemenet[i]+32;
    else eredmeny[i]=bemenet[i];
    isFirst = bemenet[i]==' ';
  }
  eredmeny[i]=0;
}
